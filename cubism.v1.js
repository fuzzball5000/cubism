(function(exports) {
  ...
  function average(data){
    ...
    function horizon(selection) {
      ...
      selection.each(function(d, i) {
        ...
        metric_.on("change.horizon-" + id, function(start, stop) {
          ...
          if (showAbovePercentile > 0) {
            var all = d.valueAll().filter(Boolean); //remove NaN etc
            var allMean = average(all);

            if (all.length > 0) {
              var sd = standardDeviation(all);
              var sdPercentile = (sd / allMean * 100);
            } else {
              }

            l = all.reduce(function(acc, val) {
              return acc + val;
            }, 0);

            var percentile = showAbovePercentile

            // if (sdPercentile > percentile || l < 0.01) {
            //  console.log(sdPercentile);
            // }
    ...
    horizon.showAbovePercentile = function(_) {
      if (!arguments.length) return showAbovePercentile;
      showAbovePercentile = _;
      return horizon;
    };
